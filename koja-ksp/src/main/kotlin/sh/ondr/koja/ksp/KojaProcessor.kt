...
class KojaProcessor(
		...
		resolver.getSymbolsWithAnnotation("sh.ondr.koja.JsonSchema")
			...
			.forEach { classDecl ->
				classDecl.qualifiedName?.asString()?.let { fqName ->
					...
					val originalLocation = classDecl.getOriginalLocation(resolver)
	...
	fun KSClassDeclaration.getOriginalLocation(resolver: Resolver): FileLocation? =
		containingFile
			?.declarations
	...
	fun KSClassDeclaration.isTopLevel(): Boolean {
		return if (origin == Origin.KOTLIN &&
			parentDeclaration != null &&
			parentDeclaration !is KSFile
		) {
			logger.error(
				"@JsonSchema-annotated class must be top-level",
				this,
			)
			false
		} else {
